...
def _parse_date(entry) -> str:
    ...
    返回 'YYYY-MM-DDTHH:MM:SS' 或空串。
    """
    import email.utils
    from datetime import timezone as _tz

    def _fmt(dt):
        """aware datetime → 统一转 UTC (2026-08-14 时区归一); naive 保持原样。"""
        if dt.tzinfo is not None:
            dt = dt.astimezone(_tz.utc)
        return dt.strftime("%Y-%m-%dT%H:%M:%S")

    # 1. feedparser 已解析 (最可靠) — struct_time 无时区, 保持 naive
    for key in ("published_parsed", "updated_parsed", "created_parsed"):
        st = entry.get(key)
        if st:
            try:
                return datetime(*st[:6]).strftime("%Y-%m-%dT%H:%M:%S")
            except Exception:
                pass
    # 2. 字符串 → RFC 822 / 常见格式
