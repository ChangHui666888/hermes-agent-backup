...
def _is_cjk(text: str) -> bool:
    """是否含 CJK 汉字 (中文聚合: C 级只放行 CJK, Step2 中文描述阈值)。"""
...
def _load_ner_payload() -> dict:
    """加载 fact_pipeline Phase 4 产物 → ner_by_article (v4.4.3 中文聚合)。

    fact_pipeline.py 对 C 级 CJK 跑 GLiNER-only, 存到 news_intel/ner_by_article.json,
    只含 CJK 文章的 key (英文 legacy 指纹源仍是 scorer 实体)。返回 {article_id: [gliner实体]}。
    """
    import json as _json
...
try:
    ...
    rows = conn.execute("""
        ...
        WHERE ni.tier IN ('A','B')
          ...
          AND rr.description IS NOT NULL AND length(rr.description) >= 80
    """).fetchall()
    for row in rows:
        desc = (row[2] or "").strip()
        html_ratio = (desc.count("<") + desc.count(">")) / max(len(desc), 1)
        # v4.4.3 中文聚合: 中文描述阈值降为 80 (中文 RSS 摘要通常 95-135 字符)
        if (len(desc) >= 200 or _is_cjk(desc)) and html_ratio < 0.3:
            conn.execute("""
...
try:
    ...
    rows = db.execute("""
        ...
        ORDER BY nc.id DESC LIMIT 500
    """).fetchall()
    # v4.4.3 中文聚合: 只放行 C 级 CJK (避免 2 万篇英文 C 级噪音), A/B 全放行
    ...
    ner_by_article = _load_ner_payload()  # v4.4.3: C 级 CJK 的 GLiNER 实体 (供 legacy 指纹)
    events = aggregate_events(rows, window_hours=48, facts_by_article=facts_by_article,
                              ner_by_article=ner_by_article)
