...
def _event_to_push_format(ev: dict) -> dict:
    ...
    """
    # v2.4 (2026-08-09): 解析本地 article_ids → 文章 URL, 供 VPS 填充 event_article (文章→事件反向关联)
    article_urls = []
    _ids = ev.get("article_ids") or []
    if _ids:
        try:
            from news_intel.db import get_db
            _db = get_db()
            _ph = ",".join("?" * len(_ids))
            _rows = _db.execute(f"SELECT article_url FROM rss_raw WHERE id IN ({_ph})", _ids).fetchall()
            _db.close()
            article_urls = [r[0] for r in _rows if r[0]]
        except Exception:
            article_urls = []
    return {
