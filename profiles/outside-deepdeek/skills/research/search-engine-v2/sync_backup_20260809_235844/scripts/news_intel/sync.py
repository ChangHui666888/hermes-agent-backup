...
def _fetch_batch(src: sqlite3.Connection, watermark: str | None, hours: int, limit: int):
    """按游标（或首次运行时按小时窗口兜底）取一批 RSS 文章，按 created_at,id 升序。

    v2.3 (2026-08-08): 复合游标 (created_at|last_id) — 修复 scanner 批量写入导致
    大量文章共享同一秒时间戳时, `created_at >= watermark` 游标被同刻重复文章占满
    LIMIT、无法跨过并列时间戳的卡死 bug (ISS-20260808-009)。
    """
    if watermark:
        parts = watermark.split("|")
        w_time = parts[0]
        w_id = parts[1] if len(parts) > 1 and parts[1] else None
        if w_id is not None:
            # 复合游标 (v2.3): (created_at > w_time) OR (created_at = w_time AND id > w_id)
            # id 为 scanner 的 TEXT hash, 字符串比较即可 (同长度 hex 字典序=数值序)
            rows = src.execute("""
                SELECT id, source, title, summary, link, category, date, created_at
                FROM rss_articles
                WHERE created_at > ? OR (created_at = ? AND id > ?)
                ORDER BY created_at ASC, id ASC
                LIMIT ?
            """, (w_time, w_time, w_id, limit)).fetchall()
            print(f"[sync] 游标续拉: (created_at,id) > ({w_time}, {w_id[:16]}...) (最多{limit}篇)")
        else:
            # 旧格式水印 (仅时间): 首次迁移时用, 避免重取已处理时间戳
            rows = src.execute("""
                SELECT id, source, title, summary, link, category, date, created_at
                FROM rss_articles
                WHERE created_at > ?
                ORDER BY created_at ASC, id ASC
                LIMIT ?
            """, (w_time, limit)).fetchall()
            print(f"[sync] 游标续拉(旧格式): created_at > {w_time} (最多{limit}篇)")
    else:
        rows = src.execute("""
            SELECT id, source, title, summary, link, category, date, created_at
            FROM rss_articles
...
def sync_recent(hours: int = 2, limit: int = 200):
    ...
    # 游标推进到本批次里最大的 (created_at, id)（哪怕文章是重复/跳过的，也要推进，
    # 否则重复文章会导致游标卡住、每次都重新扫描同一批；v2.3 用复合游标跨过同刻并列）
    max_row = max(rows, key=lambda r: (r["created_at"], r["id"]))
    max_created_at = max_row["created_at"]
    max_id = max_row["id"]
    set_sync_watermark(dst, f"{max_created_at}|{max_id}")
    stats["max_created_at"] = max_created_at  # 供 catchup 判断水印是否前进
...
def sync_catchup(limit_per_batch: int = 500, max_batches: int = 200, hours: int = 720):
    ...
    for i in range(max_batches):
        ...
        if not stats:
            break
        cur_watermark = stats.get("max_created_at")
        # 无进展保护：>= 语义下边界时间戳可能有大量同刻文章被反复重取。
        # 若水印未前进且本批无新增，说明整批都是重复 → 停止，避免死循环。
        if cur_watermark and cur_watermark == last_watermark and stats.get("total", 0) == 0:
            break
        last_watermark = cur_watermark
        if stats.get("total", 0) == 0 and stats.get("duplicate", 0) == 0:
            break
