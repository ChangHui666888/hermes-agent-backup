...
# v2.4 (2026-08-09): 硬编码 → 知识库迁移。ACTION_MAP/TOPIC_SIGNALS 改从 knowledge_base yaml 加载,
# 缺失时回退内置硬编码 (兼容)。actions.yaml 已补 patterns(en/zh) + event_type; event_types.yaml 已补 topic_signals。
_action_map_cache = None
_topic_cache = None


def _get_action_map() -> dict:
    """从 knowledge_base/actions.yaml 构建 ACTION_MAP {action_id: (event_type, patterns)}; 空则回退内置。

    v2.4: patterns 支持 dict{en,zh} 与 list(旧格式); 纳入全部带 patterns 的动作 (81+56=137)。
    event_type 优先 yaml, 缺失回退内置 ACTION_MAP。
    """
    global _action_map_cache
    if _action_map_cache is not None:
        return _action_map_cache
    m = {}
    try:
        import yaml as _y
        import os as _os
        _p = _os.path.join(_os.path.dirname(_os.path.dirname(_os.path.abspath(__file__))),
                           "..", "knowledge_base", "actions.yaml")
        for cand in [_p, "knowledge_base/actions.yaml"]:
            if _os.path.exists(cand):
                _d = _y.safe_load(open(cand, encoding="utf-8")) or {}
                for ent in (_d.get("actions") or {}).values():
                    aid = ent.get("action_id")
                    if not aid:
                        continue
                    pats = ent.get("patterns")
                    if isinstance(pats, dict):
                        pl = list(pats.get("en") or []) + list(pats.get("zh") or [])
                    elif isinstance(pats, list):
                        pl = list(pats)
                    else:
                        continue
                    if pl:
                        etype = ent.get("event_type")
                        if not etype and aid in ACTION_MAP:
                            etype = ACTION_MAP[aid][0]
                        m[aid] = (etype or "Other", pl)
                break
    except Exception:
        pass
    _action_map_cache = m if m else ACTION_MAP
    return _action_map_cache


def _get_topic_signals() -> dict:
    """从 knowledge_base/event_types.yaml 构建 TOPIC_SIGNALS {topic: en+zh关键词}; 空则回退内置。"""
    global _topic_cache
    ...
    try:
        ...
        import os as _os
        _p = _os.path.join(_os.path.dirname(_os.path.dirname(_os.path.abspath(__file__))),
                           "..", "knowledge_base", "event_types.yaml")
        for cand in [_p, "knowledge_base/event_types.yaml"]:
            if _os.path.exists(cand):
                _d = _y.safe_load(open(cand, encoding="utf-8")) or {}
                for topic, data in (_d.get("topic_signals") or {}).items():
                    m[topic] = list(data.get("en") or []) + list(data.get("zh") or [])
                break
    except Exception:
        pass
    _topic_cache = m if m else TOPIC_SIGNALS
    return _topic_cache
...
def _detect_action(text: str) -> tuple[str, str]:
    ...
    scores = {}
    for action, (etype, patterns) in _get_action_map().items():
        hits = 0
        for pat in patterns:
    ...
    return (best, _get_action_map()[best][0])
...
TOPIC_SIGNALS = {
    "Legal":       ["lawsuit","sue","court","judge","attorney","trial","ruling","verdict","legal","trade secret","indictment","plead","guilty","prosecutor", "诉讼", "起诉", "法庭", "法官", "裁决", "审判", "指控"],
    "Military":    ["war","strike","missile","troops","military","attack","drone","combat","aircraft","navy","army","bomb","weapon","invasion","raid","defense", "军事", "战争", "无人机", "导弹", "军队", "袭击", "空袭", "轰炸", "国防", "武器", "入侵", "战斗"],
    "Diplomacy":   ["ceasefire","negotiation","talks","diplomat","treaty","summit","agreement","peace deal","truce","dialogue","UN","NATO", "外交", "谈判", "协议", "会晤", "峰会", "停火", "条约", "对话"],
    "Economic":    ["tariff","sanction","trade","economy","import","export","inflation","GDP","recession","commodity","sanction","embargo", "经济", "关税", "制裁", "贸易", "出口", "进口", "出口管制", "通货膨胀", "禁运"],
    "Finance":     ["stock","earnings","revenue","profit","IPO","merger","acquisition","shares","investor","Wall Street","NASDAQ","S&P","dollar","bond","market","oil price","surge","crash","record high", "股市", "财报", "营收", "利润", "并购", "加息", "降息", "投资者", "上涨", "下跌"],
    "Politics":    ["election","vote","candidate","president","senate","congress","parliament","bill","legislation","administration","governor","mayor","govern", "选举", "投票", "总统", "议会", "政府", "法案", "议员", "候选"],
    "Technology":  ["AI","artificial intelligence","OpenAI","GPU","chip","software","startup","tech","algorithm","robot","autonomous","cyber","EV","electric vehicle", "科技", "芯片", "半导体", "光刻机", "人工智能", "软件", "机器人", "电动车", "技术"],
    "Energy":      ["oil","gas","energy","pipeline","OPEC","refinery","solar","wind","nuclear","fuel","barrel","crude","power","electricity", "石油", "能源", "天然气", "核能", "太阳能", "风能", "管道", "电力"],
    "Health":      ["covid","vaccine","hospital","disease","outbreak","pandemic","virus","drug","FDA","pharma","patient","clinical","cancer","screening", "疫苗", "病毒", "疫情", "医院", "药品", "癌症", "临床试验", "疾病"],
    "Sports":      ["World Cup","quarterfinal","semifinal","tournament","match","goal","stadium","player","team","league","championship","fans", "世界杯", "锦标赛", "比赛", "球员", "联赛", "决赛", "体育"],
    "Leadership":  ["funeral","succession","appointed","resign","chairman","CEO","executive","board","leadership","departure","nominated", "任命", "辞职", "继任", "主席", "高管", "领导层"],
    "Disaster":    ["earthquake","flood","hurricane","typhoon","tsunami","wildfire","collapse","explosion","accident","blackout", "地震", "洪水", "飓风", "台风", "海啸", "山火", "爆炸", "事故"],
}
...
def _classify_topics(text: str) -> tuple[str, str]:
    ...
    for topic, keywords in _get_topic_signals().items():
        for kw in keywords:
...
def build_fingerprint(article: dict, global_idf: dict = None, topic_idf_map: dict = None,
    ...
    return {
        ...
        "_cjk": _is_cjk(text),  # v4.4.3 跨语言匹配标记
    }


def _is_cross_lingual(fp1: dict, fp2: dict) -> bool:
    """CJK 与非 CJK 指纹对 (跨语言匹配标记)。"""
    return bool(fp1.get("_cjk")) != bool(fp2.get("_cjk"))


def _cross_lingual_score(fp1: dict, fp2: dict) -> int:
    """保守跨语言匹配 (v4.4.3): 主体精确归一相同 + 同主题 → CROSS_LINGUAL_THRESHOLD。

    仅主体精确匹配 (美国→US→United States 归一后 ==), 防止 '伊朗+军事' 之类
    主题重叠但不同故事的误合并。非强匹配回普通软分并封顶在阈值之下。
    """
    s1 = (fp1.get("subject") or "").strip()
    s2 = (fp2.get("subject") or "").strip()
    t1, t2 = fp1.get("primary_topic"), fp2.get("primary_topic")
    if s1 and s1 == s2 and (not t1 or not t2 or t1 == t2):
        return CROSS_LINGUAL_THRESHOLD
    s = 0
    ...
    if t1 and t1 == t2:
        s += 10
    return min(s, CROSS_LINGUAL_THRESHOLD - 1)
...
def fingerprint_score(fp1: dict, fp2: dict) -> int:
    ...
    v4.4.3: CJK ↔ 非CJK 走保守跨语言分支 (主体精确+同主题)
    """
    if _is_cross_lingual(fp1, fp2):
        return _cross_lingual_score(fp1, fp2)
...
# v4.4.3 保守跨语言: CJK↔非CJK 用更低的独立阈值 (主体精确+同主题才达标, 误合并风险低)
CROSS_LINGUAL_THRESHOLD = _agg_cfg("aggregate.cross_lingual_threshold", 50)
...
def _extract_action_detail(text: str, action: str) -> str | None:
    ...
    if action == "OTHER" or not text:
        return None
    patterns = _get_action_map().get(action, ("", []))[1]
    for pat in patterns:
...
def aggregate_events(articles: list[dict], window_hours: int = 24, facts_by_article: dict = None,
    ...
    for a, ts, fp in parsed:
        ...
        for ev in events:
            ...
            thr = CROSS_LINGUAL_THRESHOLD if _is_cross_lingual(fp, ev["fingerprint"]) else EVENT_THRESHOLD
        ...
        for j in range(i + 1, n):
            ...
            if ev_i["last_time"] and ev_j["start_time"]:
                if abs(ev_i["last_time"] - ev_j["start_time"]) > timedelta(hours=window_hours):
                    continue
            fs = fingerprint_score(ev_i["fingerprint"], ev_j["fingerprint"])
            mthr = CROSS_LINGUAL_THRESHOLD if _is_cross_lingual(ev_i["fingerprint"], ev_j["fingerprint"]) else MERGE_THRESHOLD
            if fs >= mthr:
                union(i, j)
    ...
    for idx, ev in enumerate(merged):
        ...
        # v4.4.3 CJK 放松: 中文事件主体是英文 canonical 名(不出现在中文标题), 保留 fact/GLiNER 主体
        _subj = fp_centroid["subject"]
        _cn_event = _is_cjk(ev["best_title"] or "") or any(_is_cjk(m.get("title") or "") for m in members)
        if _subj and not _cn_event and not _subject_is_prominent(
