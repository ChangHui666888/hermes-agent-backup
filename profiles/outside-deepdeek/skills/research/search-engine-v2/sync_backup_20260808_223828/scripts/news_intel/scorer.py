...
def score_article(
    ...
    reward = _value_reward(src, categories, market_assets, velocity_count, entities)
...
def _value_reward(source_score: int, categories: list, market_assets: list,
                  velocity_count: int, entities: dict = None) -> int:
    ...
    奖励来源:
      ...
      - 高价值实体命中 (value_tiers.json) T1一把手+8 / T2二把手+6 / T3政府机构+5 / T4核心机构+4 / T5金融巨头+6
        (GDP大国领导/政府/央行/国际组织/金融巨头; 命中多级累加, 每级计一次)
...
def _entity_value_reward(entities: dict = None) -> int:
    ...
    规则: 命中多级累加 (如 Trump[T1] + Fed[T4] = 8+4), 每级计一次, 封顶 15。
    """
    if not entities:
        return 0
    ...
    return min(reward, int(reward_cfg.get("cap", 15)))
