...
### Sketching patterns (`sketch` skill)

The `sketch` skill (now part of this umbrella) covers the "quick HTML mockups" workflow:
producing 2-3 interactive design variants side-by-side for comparison, useful when
the user wants to explore visual directions before committing to a build.

Key patterns absorbed from `sketch`:
- **Intake:** ask for feel, references, and core action before producing variants
- **Variants (2-3):** each takes a different design stance (density, emphasis, layout, aesthetic)
  — not just different accent colors
- **Each variant is a standalone HTML file** with inline CSS, system fonts or one Google Font,
  Tailwind CDN allowed, realistic fake content, and at least one meaningful interaction
- **Verify visually:** use `browser_navigate` + `browser_vision` to catch layout bugs
- **Head-to-head comparison:** after building, present a comparison table with opinionated take
- **Frontier mode:** when existing sketches exist, suggest next candidates based on consistency
  gaps, unsketched screens, state coverage, responsive gaps, interaction patterns

Load the `sketch` skill for the full detailed workflow; otherwise apply these patterns
directly under this skill's guidance.

### Popular-web-designs

Load alongside this skill when the user wants a known brand's look. 54 ready-to-paste
design systems (Stripe, Linear, Vercel, Notion, etc.).

### design-md

Use for formal DESIGN.md token spec files (YAML front matter + prose), not rendered HTML artifacts.
