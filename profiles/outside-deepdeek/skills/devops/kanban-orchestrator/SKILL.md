...
**Per-task toolsets (new).** By default a worker gets the toolsets configured in its assignee profile's `toolsets:` config. You can append additional toolsets per-task with `tools=`:

```python
kanban_create(
    title="research competitor pricing",
    assignee="<profile-A>",
    tools=["web", "browser"],   # granted ON TOP of profile defaults
    body="Scrape competitor pricing pages and compile a comparison table.",
)["task_id"]
```

This is useful when a research task needs web/browser access that the profile doesn't normally have, or when a coding task needs terminal/file access explicitly. Toolsets are **additive** — the worker's effective toolset = profile defaults + `tools` override. The dispatcher passes each name via `-t <name>` to the spawned worker.

Do NOT put toolset names in `skills=`. The `skills` field is for skill bundles (e.g. `translation`, `github-code-review`); toolset names (e.g. `web`, `browser`, `terminal`) placed in `skills` are rejected with a clear error by the kernel. Use the separate `tools` parameter instead.
...
**Confusing skills with toolsets.** The kanban system supports per-task `skills` (skill bundles loaded into the worker via `--skills`) but has **no per-task `tools`/toolsets mechanism**. All layers — CLI, tool, API, DB schema, and frontend — pass only `skills` to `create_task`. When you pass a toolset name like `"web"`, `"browser"`, or `"terminal"` as a skill, `kanban_db.create_task()` raises a clear error: _"[name] is a toolset name, not a skill name. Put toolsets in the assignee profile's `toolsets:` config instead of per-task skills."_

This means a kanban worker's available tools **always** come from the assignee profile's `toolsets:` config.yaml entry. To give a worker different tools, either:
- Configure the profile's default `toolsets:` list, or
- Create a separate profile with the right toolset mix and assign tasks there instead.

If you need per-task tool overrides, the dispatcher's `_default_spawn()` (line 6678 in `hermes_cli/kanban_db.py`) is the integration point — it would need to receive a per-task tools list and pass `--toolsets` or set `HERMES_TOOLSETS` on the worker subprocess. Currently no such parameter exists on any layer. See `references/kanban-tools-architecture.md` for the full layer trace of what would need to change.
