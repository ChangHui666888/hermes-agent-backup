...
**Per-task toolsets.** When a child task needs specific runtime capabilities, pass `tools=` to grant them on top of the assignee profile's defaults:

```python
c1 = kanban_create(
    title="scrape competitor landing pages",
    assignee="scraper-worker",
    tools=["web", "browser"],  # appended to profile defaults
)
```

This is separate from `skills=` which loads skill bundles (e.g. `translation`). Do NOT put toolset names in `skills` — the kernel rejects them with a clear error and lists the offending names.
...
- `kanban_create` ↔ `hermes kanban create "title" --assignee <profile> [--parent <id>] [--tool web --tool terminal]`
- etc.
